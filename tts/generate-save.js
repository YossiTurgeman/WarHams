...
//   Base result in Phase 1) get RED pips; faces 1, 3, 5 stay BLACK.
//   Players can tell at a glance whether the grey die triggered a base.
//   Texture is generated by tts/generate-separatist-die.js
//   and must share SEP_DIE_REV with this file for cache busting.
const SEP_DIE_REV = 1;
const SEP_DIE_TEXTURE = `${SOLDIER_BASE}/separatist_die_rev${SEP_DIE_REV}.png`;
// Standard TTS Custom_Dice (Type 1 = D6) RotationValues — maps each
// face-up world-rotation to its number for the tooltip.
const D6_ROTATION_VALUES = [
    { Value: 1, Rotation: { x: -90, y: 0, z:    0 } },
    { Value: 2, Rotation: { x:   0, y: 0, z:    0 } },
    { Value: 3, Rotation: { x:   0, y: 0, z:  -90 } },
    { Value: 4, Rotation: { x:   0, y: 0, z:   90 } },
...
[
    ...
    if (d.custom) {
        ...
        die.CustomImage = {
            ...
            ImageScalar: 1,
            WidthScale: 0,
            CustomDice: { Type: 1 /* D6 */ },
        };
        die.RotationValues = D6_ROTATION_VALUES;
        objects.push(die);
    } else {
...
// v148: tag the deck itself (in addition to each card inside) so the
// dedicated RANDOMIZE NUMBERS button can shuffle the still-undealt
// deck via deck.shuffle(). Without this the button could only find
// cards after they had already been dealt onto the hexes — clicking
// before deal raised "No number tokens found to shuffle."
numberDeck.Tags = ["number-token"];
...
const CONTAINER_NUMBER_SOUTH_ROT_Y = 0;
function makeContainer(num, role, px, py, pz) {
    ...
    const cont = baseObj("Custom_Model", `Container #${num} (${role})`,
        ...
        px, py, pz,
        // Lay the long axis east-west so the painted top number reads
        // upright from world -Z, the south edge of the table.
        { rotY: CONTAINER_NUMBER_SOUTH_ROT_Y,
...
//
// Unloading Zone board lives at (UZ_BOARD_X, UZ_BOARD_Z), directly
...
firstPlayerToken.LuaScript = [
    ...
    "        function_owner = self,",
    "        position = {0, 0.3, 0},",
    "        -- Parent tile is rotY=180; button rotation is added to",
    "        -- that, so {0,0,0} here yields combined 180 -> the label",
    "        -- reads upright from the south-facing camera.",
    "        rotation = {0, 0, 0},",
    "        width = 0,",
    "        height = 0,",
    "        font_size = 180,",
...
// IMPORTANT: keep BOARD_REV in sync with generate-planet-board.js.
...
for (let i = 0; i < 37; i++) HEX_MANIFEST.push({ kind: "terrain", url: HEX_TEX("terrain_orange_tree"), label: "Terrain Hex" });
...
for (let i = 0; i < 61; i++) {
    ...
    hex.Tags = PRODUCING_KINDS[tile.kind]
        ? ["planet-hex", "resource-hex"]
        : ["planet-hex"];
    hex.CustomImage = {
        ImageURL: tile.url,
...
function makeShuffleControl({ name, desc, x, z, tag, broadcastNoun, dealOntoTag = null }) {
    ...
    const qDeal = dealOntoTag
        ? `'${String(dealOntoTag).replace(/'/g, "\\'")}'`
        : "nil";
    tile.LuaScript = [
        `TARGET_TAG     = ${qTag}`,
        ...
        "        font_color = {0, 0, 0},",
        "        tooltip = 'Lock or unlock every ' .. BROADCAST_NOUN .. ' piece and its RANDOMIZE control.',",
        "    })",
        "    -- Reapply persisted target state after all save objects load.",
        "    Wait.frames(function() setTargetsLocked(isLocked) end, 1)",
        "end",
        "",
        "function onSave()",
        ...
        "end",
        "",
        "function setTargetsLocked(locked)",
        "    local count = 0",
        ...
        "    local targetCount = setTargetsLocked(isLocked)",
        "    self.editButton({",
        "        index = 1,",
        "        label = isLocked and 'UNLOCK' or 'LOCK',",
        "        color = isLocked and {0.55, 0.55, 0.55} or {0.95, 0.82, 0.18},",
        "    })",
        "    broadcastToAll(",
        "        isLocked and (targetCount .. ' ' .. BROADCAST_NOUN .. ' objects are LOCKED and immovable.')",
        "                  or (targetCount .. ' ' .. BROADCAST_NOUN .. ' objects are UNLOCKED and movable.'),",
        "        isLocked and {1, 0.5, 0.5} or {0.6, 1, 0.6})",
        "end",
        "",
        "-- Collect every object tagged TARGET_TAG, separated into",
        "-- still-intact Decks and loose individual cards/tiles.",
        ...
        "end",
        "",
        "-- In-place shuffle: keep targets where they are, just permute",
        "-- IDENTITIES across the existing slot positions (plus shuffle",
        "-- the deck if any cards are still inside one).",
        "function shuffleInPlace(decks, cards)",
        "    local shuffled = 0",
        "    for _, d in ipairs(decks) do",
        "        d.shuffle()",
        "        shuffled = shuffled + #d.getObjects()",
        "    end",
        "    if #cards >= 2 then",
        "        local slots = {}",
        "        for _, h in ipairs(cards) do",
        "            table.insert(slots, {pos = h.getPosition(), rot = h.getRotation()})",
        "        end",
        "        for i = #slots, 2, -1 do",
        "            local j = math.random(i)",
        "            slots[i], slots[j] = slots[j], slots[i]",
        "        end",
        "        for i, h in ipairs(cards) do",
        "            h.setPositionSmooth(slots[i].pos, false, true)",
        "            h.setRotationSmooth(slots[i].rot, false, true)",
        "        end",
        "        shuffled = shuffled + #cards",
        "    end",
        "    broadcastToAll('Randomized ' .. shuffled .. ' ' .. BROADCAST_NOUN .. '.', {0.6, 1, 0.6})",
        "end",
        "",
        "-- Deal mode: collect every tagged object (loose cards +",
        "-- existing deck) into one deck, shuffle it, then take each",
        "-- card out and place it onto a randomly-chosen target hex.",
        "-- If there are more cards than hexes, the extras stack on a",
        "-- single random hex (Catan-style 'desert + 2' overflow).",
        "function dealOntoHexes(decks, cards)",
        "    local hexes = {}",
        "    for _, obj in ipairs(getAllObjects()) do",
        "        if obj.hasTag(DEAL_ONTO_TAG) then table.insert(hexes, obj) end",
        "    end",
        "    if #hexes == 0 then",
        "        broadcastToAll('No ' .. DEAL_ONTO_TAG .. ' targets found.', {1, 0.5, 0.5})",
        "        return",
        "    end",
        "",
        "    -- Build a randomised target list: one slot per hex, then",
        "    -- repeat-round-robin extras for the overflow chits.",
        "    local order = {}",
        "    for i, h in ipairs(hexes) do order[i] = h end",
        "    for i = #order, 2, -1 do",
        "        local j = math.random(i)",
        "        order[i], order[j] = order[j], order[i]",
        "    end",
        "",
        "    -- Merge all loose cards (and any other decks) into the",
        "    -- first deck so we deal from a single shuffled source.",
        "    local sourceDeck = decks[1]",
        "    local mergeList = {}",
        "    for i = 2, #decks do table.insert(mergeList, decks[i]) end",
        "    for _, c in ipairs(cards) do table.insert(mergeList, c) end",
        "",
        "    local function dealFrom(deck)",
        "        deck.shuffle()",
        ...
        "                -- from the south camera. rotY 180 would flip it",
        "                -- upside-down for south-side players.",
        "                rotation = {0, 0, 0},",
        "                smooth = true,",
        "            })",
        "        end",
        "        broadcastToAll('Dealt ' .. total .. ' ' .. BROADCAST_NOUN ..",
        "            ' onto ' .. #order .. ' hexes.', {0.6, 1, 0.6})",
        "    end",
        "",
        "    if sourceDeck == nil then",
        "        if #cards == 0 then",
        "            broadcastToAll('No ' .. BROADCAST_NOUN .. ' found.', {1, 0.5, 0.5})",
        "            return",
        "        end",
        "        if #cards == 1 then",
        "            -- Single loose card: just plop it onto a random hex.",
        "            local hex = order[1]",
        "            local p = hex.getPosition()",
        "            cards[1].setPositionSmooth({p.x, p.y + 1.0, p.z}, false, true)",
        "            cards[1].setRotationSmooth({0, 0, 0}, false, true)",
        "            broadcastToAll('Placed 1 ' .. BROADCAST_NOUN .. '.', {0.6, 1, 0.6})",
        "            return",
        "        end",
        "        -- Multiple loose cards, no deck: group them into one.",
        "        local newDeck = group(cards)",
        "        Wait.frames(function()",
        "            local d = nil",
        "            for _, obj in ipairs(getAllObjects()) do",
        "                if obj.hasTag(TARGET_TAG) and obj.type == 'Deck' then",
        "                    d = obj; break",
        "                end",
        "            end",
        "            if d ~= nil then dealFrom(d) end",
        "        end, 20)",
        "        return",
        "    end",
        "",
        ...
        "    end",
        "",
        "    -- Merge everything into sourceDeck, then deal next frame.",
        "    table.insert(mergeList, sourceDeck)",
        "    group(mergeList)",
        "    Wait.frames(function()",
        "        local d = nil",
        "        for _, obj in ipairs(getAllObjects()) do",
        ...
        "        return",
        "    end",
        "    local decks, cards = collectTargets()",
        "    if #decks == 0 and #cards == 0 then",
        "        broadcastToAll('No ' .. BROADCAST_NOUN .. ' found to shuffle.', {1, 0.5, 0.5})",
        "        return",
        "    end",
        "    if DEAL_ONTO_TAG ~= nil then",
        "        dealOntoHexes(decks, cards)",
        "    else",
        "        if #decks == 0 and #cards < 2 then",
        "            broadcastToAll('No ' .. BROADCAST_NOUN .. ' found to shuffle.', {1, 0.5, 0.5})",
        "            return",
        "        end",
        "        shuffleInPlace(decks, cards)",
        "    end",
        "end",
    ].join("\n");
    return tile;
...
// Texture is 1400x960 (two-column). Custom_Tile Type=0
...
const extraActionsBoard = baseObj("Custom_Tile", "Squad Actions Quick Reference",
    ...
    83, 1.02, -27,
    { rotY: 180, scaleX: 7, scaleY: 0.2, scaleZ: 4.8,
      color: { r: 1, g: 1, b: 1 }, locked: true, grid: false });
...
// ─── 19. EQUIPMENT MODULE BAGS (5 slots × actual BAC modules) ───────
// v133: each bag now contains ACTUAL 3D module pieces — one per BAC
// that fits the slot — instead of generic disc tokens (v132). Each
// piece is a Custom_Model:
//   • the OBJ silhouette matches the slot type
//     (helmet / chest plate / rifle / greaves / rucksack)
//   • the Nickname is the BAC abbreviation + slot
//     (e.g. "S.A.P Chest", "H.C.A.R", "S.L.I.M.E")
//   • the colour is keyed to the BAC category so the
//     same chest mesh reads as Armor vs Tech vs Support at a glance.
// Bags live in a column 8 in west of the User Guide shelf.
//
// Meshes / texture: tts/v72/module-<slot>-rev133.obj
//                   tts/v72/module-texture-rev133.png
//   (generated by tts/generate-equipment-models.js — must match
//   MODULE_MODEL_REV in that file).
const MODULE_MODEL_REV = 133;
const MODULE_TEX_URL   = `${SOLDIER_BASE}/module-texture-rev${MODULE_MODEL_REV}.png`;
function moduleMeshURL(slot) {
    return `${SOLDIER_BASE}/module-${slot}-rev${MODULE_MODEL_REV}.obj`;
}

// Colour by BAC category so a chest plate immediately reads as
// armour vs tech vs support, etc.
const CATEGORY_COLORS = {
    "Armor":         { r: 0.55, g: 0.55, b: 0.55 },   // gunmetal grey
    "Tech":          { r: 0.32, g: 0.55, b: 0.85 },   // tech blue
    "Weapons":       { r: 0.78, g: 0.45, b: 0.16 },   // gun-finish bronze
    "Heavy Weapons": { r: 0.70, g: 0.18, b: 0.16 },   // heavy red
    "Support":       { r: 0.30, g: 0.62, b: 0.36 },   // medic green
};
...
const BACS_BY_SLOT = {
    ...
    Hands: [
        { abbr: "H.C.A.R",      category: "Weapons",
          ...
          text: "Range: 2 hexes. Add +1 to this soldier's attack die." },
        { abbr: "B.A.S.R",      category: "Heavy Weapons",
          name: "Bolt Action Sniper Rifle",
          text: "Range: 2 hexes. Pre-combat: roll 2d6 (pick higher), target 1 soldier in range. Target defends normally (1d6). Sniper skips normal combat. While defending: both sniper and target get -1 to defense die." },
        { abbr: "R.S.G",        category: "Weapons",
          name: "Repeating Shotgun",
          text: "Range: 2 hexes. If target squad is up to 1 hex away, add +2 to attack die. Otherwise add -1." },
        { abbr: "P.C.S.M.G",    category: "Weapons",
          name: "Projectile Correcting Sub Machine Gun",
          text: "Range: 2 hexes. If target squad is up to 1 hex away, roll 3d6 for this soldier's attack instead of 1d6 and pick the highest result." },
    ],
    Legs: [
...
// Two movable markers matching the First Player marker's diameter. Players
// move them from their staging positions to active combats.
for (const [index, x] of [[1, UZ_BOARD_X - 4], [2, UZ_BOARD_X]]) {
