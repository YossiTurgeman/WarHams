...
// ─── 15. NUMBER TOKENS (face-down shuffleable stack of 16) ──────────
// Catan-style chits — flat circular Custom_Tile discs with a big bold
// black number on the FRONT and a blank cream BACK (number_back.png).
// Textures generated by generate-number-tokens.js
// (tts/v<VERSION>/number_<n>.png + number_back.png).
//
// They start as a pre-shuffled face-down stack on the table so each
// player draws blind during setup. The stack is one Custom_Tile that
// owns the other 15 via ContainedObjects — TTS treats this as a real
// stack with a right-click "Shuffle" action.
const numberPool = [1, 1, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6];
// Deterministic shuffle so the JSON save is reproducible while still
// hiding the order from players.
...
const shuffledPool = shuffleSeeded(numberPool, 0xCA7AB1E);
function numberTextureURL(n) { return `${SOLDIER_BASE}/number_${n}.png`; }
const NUMBER_BACK_URL = `${SOLDIER_BASE}/number_back.png`;
function makeNumberChip(num, px, py, pz) {
    const chip = baseObj("Custom_Tile", `#${num}`,
        `Number ${num} — every resource hex marked with this token produces on a ${num}.`,
        px, py, pz,
        {
            scaleX: 1.1, scaleY: 1, scaleZ: 1.1,
            color: { r: 1, g: 1, b: 1 },
            // rotZ:180 flips the chip face-down (cream back up)
            rotX: 0, rotY: 0, rotZ: 180,
        });
    chip.CustomImage = {
        ImageURL: numberTextureURL(num),
        // Distinct back image so only the FRONT carries the digit
        ImageSecondaryURL: NUMBER_BACK_URL,
        ImageScalar: 1,
        WidthScale: 0,
        // Type 2 = circular; Stackable so the in-game stack can be
        // right-click → Shuffle'd by any player.
        CustomTile: { Type: 2, Thickness: 0.12, Stackable: true, Stretch: true },
    };
    return chip;
}
const NUM_STACK_X = 3;
