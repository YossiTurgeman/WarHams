...
// generate-number-tokens.js (tts/cards/number_<n>.png).
...
function makeNumberToken(n, stackIdx) {
    const tok = baseObj("Custom_Tile", `#${n}`,
        `Number ${n} — every resource hex marked with this token produces on a ${n}.`,
        0, 0.3 * stackIdx, 0,
        { scaleX: 1.1, scaleY: 1, scaleZ: 1.1, color: { r: 1, g: 1, b: 1 } });
    tok.CustomImage = {
        ImageURL: numberTextureURL(n),
        ImageSecondaryURL: "",
        ImageScalar: 1,
        WidthScale: 0,
...
const numTokens = [];
numberPool.forEach((num, i) => numTokens.push(makeNumberToken(num, i)));
