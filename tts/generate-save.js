...
// ─── 15. NUMBER TOKENS (face-down stack of 16) ──────────────────────
// Catan-style chits — flat circular Custom_Tile discs with a big bold
// black number printed on a cream face. Textures generated by
// generate-number-tokens.js (tts/v<VERSION>/number_<n>.png).
//
// They start as a pre-shuffled face-down stack on the table so each
// player draws blind during setup. (rotZ: 180 flips the chit so the
// blank back is up; stack position uses the same XZ for every token
// with a small Y offset so TTS auto-merges them into one stack.)
const numberPool = [1, 1, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6];
// Deterministic shuffle so saves are reproducible.
...
// Hosted under SOLDIER_BASE (the versioned path) so each CARD_VERSION
// bump produces a brand-new URL — TTS's image cache ignores ?query.
function numberTextureURL(n) { return `${SOLDIER_BASE}/number_${n}.png`; }
const NUM_STACK_X = 3;
const NUM_STACK_Z = 9;
shuffledPool.forEach((num, i) => {
    const tok = baseObj("Custom_Tile", `#${num}`,
        `Number ${num} — every resource hex marked with this token produces on a ${num}.`,
        NUM_STACK_X, 1.5 + 0.13 * i, NUM_STACK_Z,
        {
            scaleX: 1.1, scaleY: 1, scaleZ: 1.1,
            color: { r: 1, g: 1, b: 1 },
            // Flip face-down so the back is shown on the stack
    ...
    tok.CustomImage = {
        ...
        ImageScalar: 1,
        WidthScale: 0,
        // Type 2 = Circular tile; thickness gives it a real chip feel.
