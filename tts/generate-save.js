...
function makeShuffleControl({ name, desc, x, z, tag, broadcastNoun }) {
    ...
    tile.LuaScript = [
        ...
        "        return",
        "    end",
        "    -- v148: split tagged objects into two groups.",
        "    --   decks  → shuffle internally via deck.shuffle()",
        "    --   cards  → permute their world positions/rotations",
        "    -- This lets the same button work both before the deck is",
        "    -- dealt out (just shuffles the deck) and after (shuffles",
        "    -- the dealt cards' slot assignments).",
        "    local decks = {}",
        "    local cards = {}",
        ...
        "    end",
        "    local shuffled = 0",
        "    -- Shuffle any still-intact decks in place.",
        "    for _, d in ipairs(decks) do",
        "        d.shuffle()",
        "        shuffled = shuffled + #d.getObjects()",
        "    end",
        "    -- Permute the slot assignments of dealt-out individual",
        "    -- targets (cards or tiles).",
        "    if #cards >= 2 then",
        "        local slots = {}",
