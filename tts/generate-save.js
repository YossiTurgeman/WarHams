...
// ─── 19. EQUIPMENT MODULE BAGS (5 slots × 16 chips each) ────────────
// v132: 5 bags of small disc tokens — one per H.A.M.S magnet point
// (Head, Chest, Hands, Legs, Backpack). Players draw a chip from the
// matching bag and stand it in front of (or under) a soldier mini to
// show that the corresponding BAC card is currently installed in
// that slot. The bags are color-coded and live to the WEST (left) of
// the User Guide / Quick Reference shelf so they're a natural
// "equipment store" reach away from the rules text.
//
// Token texture: tts/v72/module_<slot>_rev132.png
//   (generated by tts/generate-equipment-modules.js — must match
//   MODULE_REV in that file).
const MODULE_REV = 132;
const EQUIPMENT_SLOTS = [
    { id: "head",     label: "Head",     color: { r: 0.43, g: 0.56, b: 0.76 } },
    { id: "chest",    label: "Chest",    color: { r: 0.51, g: 0.42, b: 0.31 } },
    { id: "hands",    label: "Hands",    color: { r: 0.67, g: 0.43, b: 0.26 } },
    { id: "legs",     label: "Legs",     color: { r: 0.31, g: 0.43, b: 0.29 } },
    { id: "backpack", label: "Backpack", color: { r: 0.33, g: 0.33, b: 0.38 } },
];
const MODULES_PER_SLOT  = 16;
...
EQUIPMENT_SLOTS.forEach((slot, slotIdx) => {
    ...
    const moduleURL = `${SOLDIER_BASE}/module_${slot.id}_rev${MODULE_REV}.png`;
    const moduleTokens = [];
    for (let i = 0; i < MODULES_PER_SLOT; i++) {
        // Custom_Token (cylinder/disc) cropped from the square PNG.
        // Small footprint (thickness 0.1, scale 0.45) so a chip fits
        // neatly on / next to a soldier base without dwarfing the mini.
        const tok = baseObj("Custom_Token", `${slot.label} Module`,
            `${slot.label} equipment module — magnetize to a H.A.M.S ${slot.label.toLowerCase()} slot when a matching BAC is installed.`,
            MODULE_BAG_X, 1.5 + 0.05 * i, bagZ,
            { scaleX: 0.45, scaleY: 1.0, scaleZ: 0.45, color: slot.color });
        tok.CustomImage = {
            ImageURL: moduleURL,
            ImageSecondaryURL: "",
            ImageScalar: 1.0,
        ...
        moduleTokens.push(tok);
    }
    const bag = baseObj("Bag", `${slot.label} Modules (${MODULES_PER_SLOT})`,
        `Magnetic ${slot.label} modules. Draw one when a soldier equips a ${slot.label}-slot BAC.`,
        MODULE_BAG_X, 1.5, bagZ,
        { color: slot.color });
    bag.ContainedObjects = moduleTokens;
    objects.push(bag);
});
