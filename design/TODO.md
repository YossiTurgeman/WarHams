...
- [ ] **Clarify moving onto an unconquered Separatist Base.** The current rules allow a Squad to use its first action to enter the base without triggering combat, then remain there without attacking. Define whether entering triggers the 7 base defenders immediately or requires a second Combat action, and what happens if the Squad does not attack.
