...
- [ ] **Add a combat FAQ entry for attacks from multiple hexes.** Explain that one Squad may attack while its engaged soldiers occupy multiple hexes, provided each participating soldier is in range of the targeted enemy Squad; clarify that multiple friendly Squads cannot combine into one attack, each Squad attacks only once per turn, out-of-range soldiers sit out, and squad coherency still applies.
