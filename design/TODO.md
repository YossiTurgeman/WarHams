...
- [x] **Consider moving C.A.P and prerequisite-based BACs into a Level 2 deck.** ✅ FIXED — Split the 100 BAC cards into a **Spaceport Deck** (85 cards: 17 standard types × 5) and an **Advanced BAC Deck** (15 cards: 3 C.A.P types × 5). Starting draft and Planet Bound Area use only the Spaceport Deck. At setup, one copy of each C.A.P type is placed face-up in the **Advanced BAC Area** (3 cards); the remaining 12 are shuffled as the Advanced BAC Deck. A player may only take a face-up C.A.P card from the Advanced BAC Area if they have unlocked the corresponding S.A.P type (cost: 3 resource tokens of any single type at a Spaceport/City). When a card is taken, the slot is refilled from the top of the Advanced BAC Deck. C.A.P equipping still trades in the corresponding S.A.P piece. Updated: both rulebooks (setup, acquisition, equipping, Full BAC List), game-data.json (bac_setup, deck_counts, C.A.P level/prerequisite metadata), GRAPHICS-BRIEF.md, TTS save (separate Advanced BAC Deck + Advanced BAC Area board).
