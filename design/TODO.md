...
- [x] **Add combat-location markers.** — Added three movable red circular markers with white labels, matching the First Player marker's size. They stage above the First Player token; combat declaration places a marker at the board location and returns it after resolution.
...
- [x] **Improve BAC resource-cost clarity.** — Replaced plain cost sentences with individually outlined, color-coded chips on every BAC: Oil black, Electricity yellow, Intelligence blue, Industry red, and Local Favor green. Electricity uses black text; darker chips use white text. Long costs wrap without crowding the rules text, and non-resource prerequisites use a neutral purple chip.
