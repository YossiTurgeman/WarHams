...
| Component                | Details                                                              |
| ------------------------ | -------------------------------------------------------------------- |
| **H.A.M.S Miniatures**  | Soldier miniatures in 4 player colors — **28 per player** (enough for 4 Squads of 7 H.A.M.S each). Each mini has **magnetized attachment points** for the 5 equipment slots (Head, Chest, Legs, Backpack, Hands). |
...
| **Equipment Display**  | Shared area near the board where equipped BAC cards are placed face-up with a reference module on top, visible to all players |
...
Place them randomly. One Hex will receive **two** Number Tokens (since there are 16 tokens for 15 Hexes) — that Hex produces double when either number is rolled.
...
- If combat occurred this turn: the **combat winner** (or the least-damaged side, if no clear winner) picks first.
- If no combat occurred: the **active player** picks first.
- Ties and remaining players are resolved by **turn order** (clockwise from the active player).
...
- Any **uncollected resource tokens** remaining on hexes you control (marked with your Control Hex Frame) are added to your personal supply.
- Any **BAC cards** remaining on spaceport hexes you control are added to your hand (and the matching container is removed from the board).
...
- Ranged weapons with extended range — such as **B.A.S.R** (sniper) and **S.L.I.M.E** (mortar) — may fire from beyond 2 hexes according to their card text (see **Special Weapon Rules** below). These soldiers count as engaged even if outside normal range.
...
Detach the equipment module from the struck slot and **remove it from the game** (return the module to the box).
...
| Counterattack trigger | Defense beats attack by **3 or more** |
...
| **Set Indicator** | Bottom center | Set membership icon, if applicable (S.A.P or C.A.P) |
...
| **Field Modules** | During **Phase 5** (Purchase & Equip) on your turn | P.L.A.S.T.E.R (heal damage), D.U.D.S (place bunker) |
...
| P.C.S.M.G | Projectile Correcting Sub Machine Gun | Hands | 1 Electricity, 1 Industry, 1 Intelligence | 3 | Within 1 hex: choose target, roll 2d6, pick best |
...
Move a Squad to any hex → place your colored **Control Hex Frame** on it. This applies to **resource-producing hexes**, **Spaceport Drop Zones**, **Separatist Bases** (once conquered), and **Terrain hexes**. The hex remains under your control until an opponent moves a Squad onto it and claims it.

### End-of-Round Bonus

At the **end of each round** (after all players have completed their turns), any uncollected resource tokens and BAC cards remaining on your controlled hexes are awarded to you. See the **End of Round** step in the Turn Structure for full details.
...
| 6 | Defense beats attack by **3+** = **Counterattack** |
