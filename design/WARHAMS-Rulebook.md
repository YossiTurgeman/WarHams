...
| **Equipment Modules**    | Physical miniature parts (helmets, chest plates, weapons, backpacks, leg armor) that magnetically attach to H.A.M.S minis. Each module corresponds to a BAC card type — e.g., a S.A.P (Standard Armor Plating) Helmet module, an H.C.A.R (Heavy Caliber Assault Rifle) rifle module. **5 copies** of each module type exist (matching the 5 copies of each BAC card). Each module has a **unique sculpt** per type and its **abbreviation engraved on the underside** for identification. When a BAC is equipped, its card is placed in the communal **Equipment Display** near the play area for all players to reference. The physical module on the mini is the source of truth for who has what equipped. |
